...
const AddContestPage = () => {
	...
	const handleProblemToggle = (index) => {
		setProblemList((prevProblemList) => {
			if (prevProblemList.includes(problems[index]._id)) {
				return prevProblemList.filter((p) => p !== problems[index]._id);
			} else {
				return [...prevProblemList, problems[index]._id];
			}
		});
	};
